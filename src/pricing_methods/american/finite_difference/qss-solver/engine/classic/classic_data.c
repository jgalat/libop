	  ...
	  string name)
{
  ...
  CLC_data p = (CLC_data) checkedMalloc (sizeof(*p));
  int i;
  p->states = states;
...
CLC_data
CLC_Data_ (int states, int discretes, int events, int inputs, int algebraics,
  SD_Solver solver, double ft, double dqmin, double dqrel)
{
  SET_settings settings = SET_Settings_ (solver, ft, dqmin, dqrel);
  CLC_data p = (CLC_data) checkedMalloc (sizeof(*p));
  int i;
  p->states = states;
  p->discretes = discretes;
...
CLC_IntegratorData (CLC_model clcModel, CLC_data clcData, SD_output simOutput,
  ...
  CLC_integratorData d = (CLC_integratorData) checkedMalloc (sizeof(*d));
  d->clcModel = clcModel;
  d->clcData  = clcData;
  d->simOutput = simOutput;
  d->simData  = simData;
  d->is_sampled = is_sampled;
  return d;
}
