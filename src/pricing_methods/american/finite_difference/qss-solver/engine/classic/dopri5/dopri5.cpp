...
double
Dopri5::hinit(unsigned n, FcnEqDiff fcn, double x, double * y, double posneg,
  ...
  for (i = 0; i < n; i++)
    yy1[i] = y[i] + h * f0[i];
  fcn(n, x + h, yy1, f1, integrator_data);
...
int
Dopri5::dopcor(unsigned n, FcnEqDiff fcn, double x, double * y, double xend,
  ...
  if (iout) {
    ...
    hout = h;
    xout = x;
    for (i = 0; i < (unsigned) nzc; i++)
      jroot[i] = 0;
    solout(naccpt + 1, xold, x, y, n, & irtrn, jroot, integrator_data);
    if (irtrn < 0) {
      if (fileout)
  ...
  for (i = 0; i < (unsigned) nzc; i++)
    g(i, x, y, & (val_t0[i]), integrator_data);
  ...
  while (1) {
    ...
    if (err <= 1.0) {
      ...
      // Check every discont
      for (i = 0; i < (unsigned)nzc; i++) {
        g(i, xph, y, & val_t1[i], integrator_data);
      ...
      if (events) {
        ...
        // Evaluate the roots at t0
        for (i = 0; i < (unsigned)nzc; i++)
          g(i, t0, y, & (val_t0[i]), integrator_data);
        ...
        do {
          ...
          if (dt1 < HIST * t0) {
            ...
            for (i = 0; i < (unsigned)discont_count; i++)
          ...
          for (i = 0; i < (unsigned)discont_count; i++) {
            g(discont[i], e, y, & val_e[discont[i]], integrator_data);
            #ifdef DEBUG
          ...
          if (neg_count == 0) {
            ...
            if (zero_count) {
              ...
              for (i = 0; i < (unsigned)zero_count; i++)
            ...
            for (i = 0; i < (unsigned)neg_count; i++) {
              discont[i] = neg[i];
...
Dopri5::Dopri5(void *int_data) {
  nfcn = nstep = naccpt = nrejct = 0;
  yy1 = k1 = k2 = k3 = k4 = k5 = k6 = ysti = yold = NULL;
  rcont1 = rcont2 = rcont3 = rcont4 = rcont5 = NULL;
  indir = NULL;
  val_t0 = val_t1 = val_e = NULL;
  neg = discont = zero = NULL;

  integrator_data = int_data;
}

/* front-end */
int
Dopri5::run (unsigned n, FcnEqDiff fcn, double x, double* y, double xend,
...
double
Dopri5::contd5 (unsigned ii, double x)
{
  unsigned i;
  double theta, theta1;
