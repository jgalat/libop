...
SD_SimulationSettings ()
{
  SD_simulationSettings p = (SD_simulationSettings) checkedMalloc (sizeof(*p));
...
SD_SimulationLog (char *name)
{
  ...
  SD_simulationLog p = (SD_simulationLog) checkedMalloc (sizeof(*p));
  p->states = NULL;
  p->handlers = NULL;
...
{
  SD_eventData p = (SD_eventData) checkedMalloc (size * sizeof(*p));
  int i;
  for (i = 0; i < size; i++)
    {
      p[i].nLHSSt = 0;
...
{
  SD_parameters p = (SD_parameters) checkedMalloc (sizeof(*p));
  p->derDelta = derDelta;
  p->zcHyst = zcHyst;
  p->minStep = minStep;
  p->symDiff = symDiff;
...
{
  int i;
  SD_sampledOutput p = (SD_sampledOutput) checkedMalloc (sizeof(*p));
...
{
  SD_outputVariable p = (SD_outputVariable) checkedMalloc (outputs * sizeof(*p));
  return (p);
}
...
SD_Statistics ()
{
  SD_statistics p = (SD_statistics) checkedMalloc (sizeof(*p));
    p->initializeLPS = 0;
...
{
  stats->simulationTimes = (double*) checkedMalloc (lps * sizeof(double));
  cleanDoubleVector (stats->simulationTimes, 0, lps);
  stats->simulationMessages = (long unsigned int*) checkedMalloc (lps * sizeof(unsigned long));
