...
{
  node n = Node (ns, ord);
  list l = (list) checkedMalloc (sizeof(*l));
  l->begin = n;
  l->end = n;
  l->nodeSize = ns;
  l->order = ord;
...
Vector (int cant)
{
  vector v = (vector) checkedMalloc (sizeof(*v));
...
{
  IBX_inbox p = (IBX_inbox) checkedMalloc (sizeof(*p));
  if (ack == 0)
    {
...
MLB_Mailbox (int lps)
{
  MLB_mailbox p = (MLB_mailbox) checkedMalloc (sizeof(*p));
