...
SET_Settings (char *fname)
{
  ...
  SET_settings p = (SET_settings) checkedMalloc (sizeof(*p));
  p->minstep = 0;
  p->zchyst = 0;
  p->derdelta = 0;
  p->it = 0;
  ...
  p->dqmin = (double*) checkedMalloc (count * sizeof(double));
  for (n = 0; n < count; n++)
    {
  ...
  count = config_setting_length (lists);
  p->dqrel = (double*) checkedMalloc (count * sizeof(double));
  for (n = 0; n < count; n++)
    {
...
{
  SET_settings p = (SET_settings) checkedMalloc (sizeof(*p));
  p->minstep = MIN_STEP; // default ini
  p->zchyst = ZC_HYST; // default ini
  p->derdelta = DER_DELTA; // default ini
  p->it = 0; // default ini
  p->ft = ft; // arg
  p->dt = 0; // default ~ini
  p->symdiff = 1; // default ini
  p->lps = LPS; // default ini
  p->nodesize = NODE_SIZE; // default ini
  p->solver = solver; // arg
  p->order = _getOrder(solver);
  ...
  p->nDQMin = 1;
  p->nDQRel = 1;
  p->dqmin = (double*) checkedMalloc(sizeof(double));
  p->dqrel = (double*) checkedMalloc(sizeof(double));
  p->dqmin[0] = dqmin; // arg
  p->dqrel[0] = dqrel; // arg 
